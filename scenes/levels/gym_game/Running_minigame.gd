...
@onready var running_minigame: RunningMinigame = $RunningMinigame
...
func _ready():
	...
	running_minigame.minigame_completed.connect(
...
func increment_seq_idx():
	...
	if dialog_sequence_idx == 2:
		trainer_dialogue.dialog_sequence.connect(
			...
			func(): (
				create_tween()
				. tween_callback(func(): running_minigame.level_in_progress = true)
				. set_delay(0.75)
			)
		)
